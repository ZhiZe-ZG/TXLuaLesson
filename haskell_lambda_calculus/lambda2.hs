...
-- 考虑换用这种写法data My = Variables:.Expressions deriving(Show)
-- data My = Expressions:=>Expressions deriving(Show)
-- VExp简写为VE
-- 可以在输入输出的时候通过字符串替换把VE去掉或者加上
...
-- getNotUsedVar s = Var ((\x -> x+1) (varNum (head (Set.toDescList s))))
-- 这里写一个不用varNum的版本
getNotUsedVar s = Var ((\x -> x+1) ((\(Var y) -> y) (head (Set.toDescList s))))
