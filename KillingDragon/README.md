...
1. 用haskell构造lambda calculus。重点在于介绍lambda caluclus的计算规则，haskell只当作一个字符串解析器。
1. 从lambda calculus介绍函数式编程，从函数式编程介绍到haskell
1. 用haskell构造各种数学的形式系统。顺便讲讲数学。
...
* haskell只是一个帮助我们进行文字替换的工具（先这样理解，等后续lambda表达式实现了，再学Haskell的详尽语法）
不考虑抽象意义，只管文字替换
...
由于我不想浪费纸笔，也不想亲手去抄写。我们在Lua中通过字符串操作来进行这个游戏。当然了，如果你想炫耀一下新买的手账和钢笔，也可以通过纯手工的方式玩这个游戏。

距离我写完Lua系列教程已经有一段时间了。

柠檬76今天和我说想看C教程。于是我就顺便重写了这篇文章。

C语言中有一个现在看来有点奇葩的功能——编译预处理命令。





我本来想从一个简单且幼稚的小学数学题入手。但是思前想后，我决定不这么做。这道题长这样：
$$
\square + \square + \square = \triangle  \\
\square + \square +\triangle = \bigcirc  \\
\bigcirc + \square = \triangle + ?
$$
就这样一道题，我还要从符号替换的角度分析半天，实在是太累了。况且说半天，读者也不一定明白我要表达什么。

正好柠檬76说她想看C的教程。不如我们今天就从C入手。

所以我们抛开这道题，直接切入今天的正题——一个简单的文字游戏。



这里有一段文字“大家好我是酒石酸。今天我酒石酸给大家带来了新一期的视频。”。请把其中的“酒石酸”全部替换为“太玄”。

我知道这对你来说很简单。结果就是“大家好我是太玄。今天我太玄给大家带来了新一期的视频。”。

让我们增加一下难道。这次的段落是“酒石酸制作了一期我的世界视频。”。把
...
最经典的文字游戏玩法，当然是亲自用纸笔抄抄写写，勾勾画画。但是我不想把大把时间消耗在抄写字符上。我想把这些枯燥、机械的动作交给别人来处理，而我只关注游戏中比较有意思的部分。幸好，我的这个需求，在将近一百年前就有解决方案了——做一台机器，让机器按照我的指引完成抄写。
...
> 以上据的几个例子都是不带输入参数的值构造器。还有带参数和输入的构造器。例如`(a,b)`，接受两个值然后返回由它们组成的元组（元组是一种数据结构，一个元组也就是一个值）。
